 ...
 * @param {Array} datasets - Array de datasets do gráfico
...
function createInteractiveLegend(chartId, containerId, datasets, options = {}) {
  ...
  const updateChart = () => {
    ...
    const visibleDatasets = datasets.filter((ds, idx) => {
      ...
      return window[visibilityKey][label] !== false;
    });
  ...
  const renderLegend = () => {
    const total = datasets.reduce((sum, ds) => {
      const data = ds.data || [];
      return sum + data.reduce((s, v) => s + (v || 0), 0);
    }, 0);

    container.innerHTML = datasets.map((ds, idx) => {
      const label = ds.label || `Dataset ${idx}`;
      const isVisible = window[visibilityKey][label] !== false;
      let color = ds.borderColor || ds.backgroundColor || getColorFromPalette(idx);
      ...
      const data = ds.data || [];
      const datasetTotal = data.reduce((sum, v) => sum + (v || 0), 0);
    ...
    container.querySelectorAll('.legend-item').forEach((item) => {
      item.addEventListener('click', () => {
        ...
        window[visibilityKey][label] = !window[visibilityKey][label];
        updateChart();
        renderLegend();
      });
    });
    ...
    if (btnDesmarcar) {
      btnDesmarcar.addEventListener('click', (e) => {
        ...
        datasets.forEach((ds, idx) => {
          const label = ds.label || `Dataset ${idx}`;
          window[visibilityKey][label] = false;
        });
        updateChart();
        renderLegend();
      });
  ...
  return {
    update: updateChart,
    render: renderLegend,
    getVisibility: () => ({ ...window[visibilityKey] }),
    setVisibility: (label, visible) => {
      window[visibilityKey][label] = visible;
      updateChart();
      renderLegend();
    }
  };
 ...
 * @param {Array} colors - Array de cores (opcional)
 * @param {Object} options - Opções de configuração
 */
function createDoughnutLegend(chartId, containerId, labels, values, colors = null, options = {}) {
  ...
  if (!colors && chart.data.datasets && chart.data.datasets[0]) {
    colors = chart.data.datasets[0].backgroundColor || chart.data.datasets[0].borderColor || [];
  }
  ...
  const updateChart = () => {
    ...
    const visibleLabels = labels.filter(label => window[visibilityKey][label] !== false);
...
if (typeof window !== 'undefined') {
  if (!window.chartLegend) window.chartLegend = {};
