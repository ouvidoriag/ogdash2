...
# ===================================================================================
# 5.5) BACKFILL TEMPORÁRIO DE 'servidor' (REMOVER APÓS 1ª EXECUÇÃO)
# ===================================================================================
_BANNER("5.5) BACKFILL TEMPORÁRIO DE 'servidor' (REMOVER DEPOIS)")
print(" Executando backfill para aplicar o dicionário de servidores a TODA a base tratada...")
logging.info("INICIANDO: Backfill temporário da coluna 'servidor' usando o dicionário do Item 7.5.")
...
try:
    ...
    if 'protocolo' not in df_tratada_hist.columns or 'servidor' not in df_tratada_hist.columns:
        raise ValueError("Colunas 'protocolo' ou 'servidor' não encontradas na planilha tratada.")

    # 2. REPLICA A LÓGICA DE TRATAMENTO DO ITEM 7.5
    _SUB("Aplicando a lógica do 'dicionario_servidor'...")
    dicionario_servidor = {
    ...
    _orig = df_tratada_hist["servidor"].astype(str).str.strip()
    df_tratada_hist["servidor_tratado"] = _orig.map(dicionario_servidor).fillna(_orig)

    # 3. IDENTIFICA APENAS AS LINHAS QUE PRECISAM SER MUDADAS
    _SUB("Identificando registros que precisam de atualização...")
    df_para_atualizar = df_tratada_hist[df_tratada_hist["servidor"] != df_tratada_hist["servidor_tratado"]]
    
    if df_para_atualizar.empty:
        print("✅ Nenhuma correção necessária. A coluna 'servidor' já está padronizada.")
        logging.info("Backfill: Nenhuma correção de servidor necessária.")
    else:
        print(f" Encontradas {len(df_para_atualizar)} linhas que serão padronizadas na coluna 'servidor'.")
        logging.info(f"Backfill: Encontradas {len(df_para_atualizar)} linhas de 'servidor' para padronizar.")

        # 4. PREPARA E EXECUTA A ATUALIZAÇÃO EM LOTE
        _SUB("Preparando e enviando a atualização para o Google Sheets...")
        TARGET_COL_INDEX = df_tratada_hist.columns.get_loc('servidor') + 1
        ...
        for _, row in df_para_atualizar.iterrows():
            ...
            servidor_tratado = row['servidor_tratado'] # Pega o valor novo, já tratado
            if protocolo in protocolo_para_linha:
                linha_idx = protocolo_para_linha[protocolo]
                cells_to_update.append(gspread.Cell(row=linha_idx, col=TARGET_COL_INDEX, value=str(servidor_tratado)))

        if cells_to_update:
            aba_tratada.update_cells(cells_to_update, value_input_option='USER_ENTERED')
            print(f"✅ Backfill da coluna 'servidor' concluído. {len(cells_to_update)} células foram padronizadas.")
            print(" AVISO: Lembre-se de remover todo o bloco 'Item 5.5' do script após esta execução.")
            logging.info(f"CONCLUÍDO: Backfill. {len(cells_to_update)} células de 'servidor' foram padronizadas.")
        else:
            print(" Nenhuma célula pôde ser mapeada para atualização.")

except Exception as e:
    print(f"❌ Erro crítico durante o backfill: {e}")
    logging.error(f"Erro crítico durante o backfill do servidor: {e}", exc_info=True)
