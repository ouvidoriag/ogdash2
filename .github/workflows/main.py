...
try:
    ...
    PLANILHA_TRATADA_ID = "1-kuT4GFl_Vtc1xjQ2GJuNLNGSLUriftV75IgtbgeNxo"  # <-- coloque aqui o ID CORRETO da planilha tratada fixa
...
# ===================================================================================
# 5.5) RESTAURAÇÃO TEMPORÁRIA DE 'unidade_cadastro' (REMOVER APÓS 1ª EXECUÇÃO)
# ===================================================================================
_BANNER("5.5) RESTAURAÇÃO TEMPORÁRIA DE 'unidade_cadastro' (REMOVER DEPOIS)")
print(" Executando restauração de 'unidade_cadastro' a partir da base bruta...")
logging.info("INICIANDO: Restauração da coluna 'unidade_cadastro'.")

try:
    _SUB("Carregando e preparando dados para a restauração...")
    ...
    # Garante que não haja protocolos duplicados na fonte da verdade
    ...
    # Cria o mapa {protocolo: unidade_cadastro_original_da_bruta}
    mapa_unidade_bruta = df_bruta_mapa.set_index('protocolo')['unidade_cadastro'].to_dict()

    _SUB("Identificando protocolos da bruta que precisam de sincronização na tratada...")
    ...
    df_tratada_subset['unidade_original_bruta'] = df_tratada_subset['protocolo'].map(mapa_unidade_bruta)
    
    # Compara a coluna original com a da bruta para encontrar o que precisa ser atualizado
    df_para_atualizar = df_tratada_subset[
        df_tratada_subset['unidade_cadastro'].astype(str) != df_tratada_subset['unidade_original_bruta'].astype(str)
    ]
    
    if not df_para_atualizar.empty:
        print(f" -> Encontradas {len(df_para_atualizar)} linhas para restaurar na coluna 'unidade_cadastro'.")
        logging.info(f"Backfill: {len(df_para_atualizar)} linhas de 'unidade_cadastro' para restaurar.")
        ...
        for _, row in df_para_atualizar.iterrows():
            ...
            unidade_original = row['unidade_original_bruta']
            if protocolo in protocolo_para_linha:
                linha_idx = protocolo_para_linha[protocolo]
                cells_to_update.append(gspread.Cell(row=linha_idx, col=TARGET_COL_INDEX, value=str(unidade_original)))

        if cells_to_update:
            aba_tratada.update_cells(cells_to_update, value_input_option='USER_ENTERED')
            print(f"✅ Restauração concluída. {len(cells_to_update)} células foram atualizadas.")
            logging.info(f"CONCLUÍDO: Restauração. {len(cells_to_update)} células de 'unidade_cadastro' foram atualizadas.")
    else:
        print("✅ Nenhuma divergência encontrada. Os protocolos da base bruta já estão sincronizados na tratada.")
        logging.info("Backfill: Nenhuma restauração de 'unidade_cadastro' necessária.")

except Exception as e:
    print(f"❌ Erro crítico durante a restauração de 'unidade_cadastro': {e}")
    logging.error(f"Erro crítico durante a restauração de 'unidade_cadastro': {e}", exc_info=True)
...
try:
    ...
    PLANILHA_TRATADA_ID = "1-kuT4GFl_Vtc1xjQ2GJuNLNGSLUriftV75IgtbgeNxo"
...
if df_send.empty:
    ...
    sheet_is_empty = (aba_tratada.row_count == 0) or (len(aba_tratada.get_all_values()) <= 1 and not any(aba_tratada.get_all_values()[0]))
