 ...
 * ATUALIZADO: Respeita seção ativa (Ouvidoria/Zeladoria)
 */
function initPage() {
  ...
  // Ativar botão home da seção correta
...
function getPageLoader(page) {
  ...
  if (page === 'zeladoria-home') {
    // Página home da Zeladoria (estática por enquanto)
    return () => {
      if (window.Logger) {
        window.Logger.info('Página Home Zeladoria carregada');
      } else {
        console.log('🏗️ Página Home Zeladoria carregada');
      }
      return Promise.resolve();
    };
  }
  if (page === 'zeladoria-overview') {
    // Página overview da Zeladoria (placeholder por enquanto)
    return () => {
      if (window.Logger) {
        window.Logger.info('Página Overview Zeladoria carregada');
      } else {
        console.log('📊 Página Overview Zeladoria carregada');
      }
      return Promise.resolve();
    };
  }
...
const preloadManager = {
  ...
  async preloadOtherPages(currentPage) {
    // OTIMIZAÇÃO: Aguardar mais tempo antes de começar (dar tempo para página atual carregar completamente)
    await new Promise(resolve => {
      if (window.timerManager) {
        window.timerManager.setTimeout(resolve, 15000, 'preload-delay'); // 15 segundos (era 3)
      } else {
...
async function loadSection(page) {
  ...
  // OTIMIZAÇÃO: Carregar data-pages.js apenas se página legado for acessada
  if (window.legacyLoader?.isLegacyPage(page)) {
    await window.legacyLoader.loadIfNeeded(page).catch(() => {
      // Ignorar erro - módulo pode já estar carregado via script tag
      if (window.Logger) {
        window.Logger.debug('Módulo legado pode já estar carregado');
      }
    });
  }
  ...
  try {
    ...
    if (loader) {
      // DEBUG: Log para verificar quando página é carregada
      if (window.Logger) {
        window.Logger.debug(`📄 Carregando página: ${page}`);
      } else {
      ...
      loader().then(() => {
        if (window.Logger) {
          window.Logger.debug(`✅ Página ${page} carregada com sucesso`);
        }
...
/**
 * Gerenciar troca de seção (Ouvidoria/Zeladoria)
 */
function initSectionSelector() {
  ...
  function switchSection(section) {
    // Atualizar botões
    btnOuvidoria.classList.toggle('active', section === 'ouvidoria');
    btnZeladoria.classList.toggle('active', section === 'zeladoria');
    
    // Atualizar título
    if (sectionTitle) {
      sectionTitle.textContent = section === 'ouvidoria' ? 'Ouvidoria' : 'Zeladoria';
    }
    
    // Mostrar/ocultar menus
    menuOuvidoria.style.display = section === 'ouvidoria' ? 'block' : 'none';
    menuZeladoria.style.display = section === 'zeladoria' ? 'block' : 'none';
    ...
    // Mostrar página inicial da seção
    const homePageId = section === 'ouvidoria' ? 'page-home' : 'page-zeladoria-home';
    ...
    const activeMenu = section === 'ouvidoria' ? menuOuvidoria : menuZeladoria;
    activeMenu.querySelectorAll('div[data-page]').forEach(b => b.classList.remove('active'));
    const homeBtn = activeMenu.querySelector(`div[data-page="${section === 'ouvidoria' ? 'home' : 'zeladoria-home'}"]`);
    if (homeBtn) {
      homeBtn.classList.add('active');
    }
  }
  ...
  // Event listeners
  btnOuvidoria.onclick = () => switchSection('ouvidoria');
  btnZeladoria.onclick = () => switchSection('zeladoria');
  
  // Inicializar com Ouvidoria
...
function initNavigation() {
  ...
  // Função para lidar com cliques em páginas (funciona para ambos os menus)
  function handlePageClick(btn) {
    const pageName = btn.getAttribute('data-page');
    if (!pageName) return;
    
    // Encontrar o menu pai (Ouvidoria ou Zeladoria)
    const menu = btn.closest('nav');
    if (!menu) return;
    
    // Atualizar estado visual IMEDIATAMENTE (sem esperar carregamento)
    menu.querySelectorAll('div[data-page]').forEach(b => b.classList.remove('active'));
  ...
  // Adicionar listeners para ambos os menus
  const menuOuvidoria = document.getElementById('sideMenuOuvidoria');
  const menuZeladoria = document.getElementById('sideMenuZeladoria');
  
  if (menuOuvidoria) {
    menuOuvidoria.querySelectorAll('div[data-page]').forEach(btn => {
      btn.onclick = () => handlePageClick(btn);
    });
  }
  
  if (menuZeladoria) {
    menuZeladoria.querySelectorAll('div[data-page]').forEach(btn => {
      btn.onclick = () => handlePageClick(btn);
    });
  }
}
...
function init() {
  ...
  initSectionSelector(); // NOVO: Inicializar seletor de seção primeiro
  initPage();
  initNavigation();
  initEventListeners();
