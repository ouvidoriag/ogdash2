...
async function renderKpisImplementationFull(sum, dailyData, byMonth) {
  try {
    ...
    // SLA chart
    // CORREÇÃO: Adicionar tratamento de erro para não bloquear renderização
    let sla = { concluidos: 0, verdeClaro: 0, amarelo: 0, vermelho: 0 };
    try {
      const fetchJSONFn = typeof fetchJSON === 'function'
        ? fetchJSON
        : (window.api?.fetchData || window.dataLoader?.load || (async (url) => {
            const res = await fetch(url);
            if (!res.ok) throw new Error('Request failed');
            return res.json();
          }));
      sla = await fetchJSONFn('/api/sla/summary') || sla;
    } catch (e) {
