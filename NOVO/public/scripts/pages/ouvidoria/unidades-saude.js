...
/**
 * Atualizar KPIs da página Unidades de Saúde
 */
function updateUnidadesSaudeKPIs(assuntos, tipos) {
  const total = assuntos?.reduce((sum, item) => sum + (item.quantidade || item.count || 0), 0) || 0;
  const assuntosUnicos = assuntos?.length || 0;
  const tiposUnicos = tipos?.length || 0;
  ...
  const kpiMaisComum = document.getElementById('kpiAssuntoMaisComumUnidade');
  ...
  if (kpiTipos) kpiTipos.textContent = tiposUnicos.toLocaleString('pt-BR');
  if (kpiMaisComum) {
