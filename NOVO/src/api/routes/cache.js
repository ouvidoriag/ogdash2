...
export default function cacheRoutes(prisma) {
   ...
   */
  router.get('/status', (req, res) => cacheController.getCacheStatus(req, res, prisma));
  
  /**
   * GET /api/cache/universal
   * Cache universal (desabilitado por padrão)
   */
  router.get('/universal', (req, res) => cacheController.getUniversal(req, res, prisma));
  
  /**
   * POST /api/cache/rebuild
   * Reconstruir cache universal manualmente
   */
  router.post('/rebuild', (req, res) => cacheController.rebuildCache(req, res, prisma));
  
  /**
   * POST /api/cache/clean-expired
   * Limpar entradas de cache expiradas
   */
  router.post('/clean-expired', (req, res) => cacheController.cleanExpired(req, res, prisma));
  
  /**
   * POST /api/cache/clear-all
   * Limpar todo o cache (memória + banco de dados)
   */
  router.post('/clear-all', (req, res) => cacheController.clearAll(req, res, prisma));
  
  /**
   * POST /api/cache/clear
   * Limpar cache em memória (compatibilidade)
   */
  router.post('/clear', (req, res) => cacheController.clearMemory(req, res, prisma));
