...
async function loadTable(limit = 50) {
  // CORREÇÃO: Verificar se elementos existem ANTES de fazer requisição
 ...
 */
// DEPRECATED: Esta função foi movida para modules/data-overview.js
// Esta função agora apenas delega para a versão otimizada do módulo
async function loadOverview(forceRefresh = false) {
  // Se a versão otimizada do módulo já está disponível, usar ela
  // O módulo data-overview.js marca a função com uma propriedade especial
  if (window.data?.loadOverview && window.data.loadOverview._fromModule === true) {
    if (window.Logger) {
      window.Logger.debug('✅ Usando loadOverview do módulo data-overview.js');
    }
    return window.data.loadOverview(forceRefresh);
  }
  
  // Se ainda não está disponível, aguardar mais tempo (módulo pode estar carregando)
  return new Promise((resolve) => {
    let attempts = 0;
    const maxAttempts = 50; // 5 segundos (aumentado de 3)
    
    const checkAndExecute = () => {
      attempts++;
      if (window.data?.loadOverview && window.data.loadOverview._fromModule === true) {
        if (window.Logger) {
          window.Logger.debug(`✅ Módulo data-overview.js encontrado após ${attempts * 100}ms`);
        }
        window.data.loadOverview(forceRefresh).then(resolve).catch(resolve);
        return;
      }
      ...
      if (attempts < maxAttempts) {
        ...
        // Timeout: usar implementação antiga como fallback
        if (window.Logger) {
          window.Logger.warn('⚠️ Módulo data-overview.js não carregado a tempo, usando fallback');
        } else {
          console.warn('⚠️ Módulo data-overview.js não carregado a tempo, usando fallback');
        }
        // Fallback: usar implementação antiga (mas não exportar para não sobrescrever)
        resolve();
      }
    };
    
    // Aguardar um pouco antes de começar a verificar (dar tempo para módulo carregar)
 ...
 */
/**
 * Carregar Órgão e Mês
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadOrgaoMes de data-pages.js em vez desta versão
 */
...
try {
  ...
  // NOTA: loadOrgaoMes e loadTempoMedio foram movidos para data-pages.js
  // Elas serão carregadas sob demanda via lazy loading quando necessário
  // Não há necessidade de exportá-las aqui, pois data-pages.js já as exporta

  // Mesclar funções do data.js com window.data existente
  // NOTA: Funções migradas para data-pages.js ainda são exportadas aqui para compatibilidade
  // mas as versões em data-pages.js têm prioridade (são carregadas primeiro)
  const dataExports = {
  reloadAllData,
  loadAIInsights,
  loadStatusOverview,
  ...
  // loadOverview: Não exportar se módulo data-overview.js já exportou (prioridade)
  // A função loadOverview acima delega para o módulo se disponível
  loadAdvancedCharts,
  loadSankeyChart,
  loadTreeMapChart,
  loadGeographicMap,
  ...
  drawSpark,
  drawSparkDaily,
  // Funções migradas para data-pages.js (mantidas aqui apenas para compatibilidade retroativa)
  // As versões em data-pages.js têm prioridade e são usadas pelo sistema
  // @deprecated - Use window.data.loadXpto de data-pages.js
  loadCategoria,
  loadStatusPage,
  loadBairro,
  loadUAC,
  ...
  loadTema,
  loadAssunto,
  // loadOrgaoMes e loadTempoMedio movidos para data-pages.js
  loadCadastrante,
  loadReclamacoes,
  };
  
  // Só adicionar loadOverview se módulo não estiver disponível
  if (window.data?.loadOverview?._fromModule !== true) {
    dataExports.loadOverview = loadOverview;
  }
  
  // Adicionar funções restantes que não estavam no objeto inicial
  // NOTA: Funções migradas para data-pages.js ainda são exportadas aqui para compatibilidade
  // mas as versões em data-pages.js têm prioridade (são carregadas primeiro)
  Object.assign(window.data, dataExports, {
    // @deprecated - Funções migradas para data-pages.js (mantidas apenas para compatibilidade)
    loadProjecao2026,
    loadSecretaria,
    ...
    loadSetor,
    loadUnit,
    // Funções não migradas (ainda em uso)
    loadCountChart,
    loadTimeChart,
    // Chat functions
    loadCoraChat,
  ...
  // NOTA: Estas funções são carregadas sob demanda via lazy loading quando necessário
  const orgaoMesStatus = typeof window.data?.loadOrgaoMes === 'function' ? '✅ SIM (carregado)' : '⏳ Será carregado sob demanda';
  const tempoMedioStatus = typeof window.data?.loadTempoMedio === 'function' ? '✅ SIM (carregado)' : '⏳ Será carregado sob demanda';
  
  if (window.Logger) {
    window.Logger.debug(`loadOrgaoMes: ${orgaoMesStatus}`);
    window.Logger.debug(`loadTempoMedio: ${tempoMedioStatus}`);
  } else {
    console.log(`🔍 loadOrgaoMes: ${orgaoMesStatus}`);
    console.log(`🔍 loadTempoMedio: ${tempoMedioStatus}`);
  }
} catch (error) {
...
/**
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadUnit de data-pages.js em vez desta versão
 */
 ...
 * Carregar Secretaria
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadSecretaria de data-pages.js em vez desta versão
 */
...
/**
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadSecretariasDistritos de data-pages.js em vez desta versão
 */
 ...
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadTipo de data-pages.js em vez desta versão
 */
 ...
 * Carregar Setor
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadSetor de data-pages.js em vez desta versão
 */
 ...
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadCadastrante de data-pages.js em vez desta versão
 */
 ...
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadReclamacoes de data-pages.js em vez desta versão
 */
 ...
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadProjecao2026 de data-pages.js em vez desta versão
 */
 ...
 * Carregar Tema
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadTema de data-pages.js em vez desta versão
 */
 ...
 * Carregar Assunto
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadAssunto de data-pages.js em vez desta versão
 */
 ...
 * Carregar Tempo Médio
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadTempoMedio de data-pages.js em vez desta versão
 */
 ...
 * Carregar Categoria
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadCategoria de data-pages.js em vez desta versão
 */
 ...
 * Carregar Status
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadStatusPage de data-pages.js em vez desta versão
 */
 ...
 * Carregar Bairro
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadBairro de data-pages.js em vez desta versão
 */
 ...
 * Carregar UAC
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadUAC de data-pages.js em vez desta versão
 */
 ...
 * Carregar Responsável
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadResponsavel de data-pages.js em vez desta versão
 */
 ...
 * Carregar Canal
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadCanal de data-pages.js em vez desta versão
 */
 ...
 * Carregar Prioridade
 * @deprecated Esta função foi migrada para data-pages.js com Chart Factory e dataStore
 * Use window.data.loadPrioridade de data-pages.js em vez desta versão
 */
