...
function renderTempoMedioStats(stats) {
  ...
  if (statMedia) {
    statMedia.textContent = (stats.media || stats.average || stats.dias || 0).toFixed(1);
  }
  if (statMediana) {
...
async function renderTempoMedioCharts(stats, dataMes) {
  // Carregar dados por órgão/unidade (usar endpoint correto)
  const dataOrgao = await window.dataLoader?.load('/api/stats/average-time', {
      useDataStore: true,
      ttl: 5 * 60 * 1000
    }) || [];
  ...
  if (dataMes && Array.isArray(dataMes) && dataMes.length > 0) {
    ...
    const values = last12.map(m => m.dias || m.average || m.media || 0);
...
async function loadSecondaryTempoMedioData() {
  try {
    ...
    if (dataUnidade && Array.isArray(dataUnidade) && dataUnidade.length > 0) {
      ...
      const labels = top10.map(u => u.unit || u.org || u._id || 'N/A');
      const values = top10.map(u => u.dias || u.average || u.media || 0);
