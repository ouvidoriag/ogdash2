  ...
  return withCache(key, 300, res, async () => {
    ...
    try {
      ...
      const [last7, last30] = await Promise.all([
        prisma.record.count({
          where: { ...where, dataCriacaoIso: { gte: last7Str, lte: todayStr } }
        }),
        prisma.record.count({
          where: { ...where, dataCriacaoIso: { gte: last30Str, lte: todayStr } }
        })
      ]);
      
      // Formatar dados
      const statusCounts = byStatus
        .map(r => ({ status: r.status ?? 'Não informado', count: r._count._all }))
        .sort((a, b) => b.count - a.count);
      
      const manifestationsByMonth = byMonth.map(x => ({
        month: x.ym || x.month || '',
        count: x.count || 0
      }));
      
      const manifestationsByDay = byDay;
      
      const manifestationsByTheme = byTheme
        .map(r => ({ theme: r.tema ?? 'Não informado', quantidade: r._count._all }))
        .sort((a, b) => b.quantidade - a.quantidade);
      
      const manifestationsBySubject = bySubject
        .map(r => ({ assunto: r.assunto ?? 'Não informado', quantidade: r._count._all }))
        .sort((a, b) => b.quantidade - a.quantidade);
      
      const manifestationsByOrgan = byOrgaos
        .map(r => ({ key: r.orgaos ?? 'Não informado', count: r._count._all }))
        .sort((a, b) => b.count - a.count);
      
      // Por secretaria (usar mesmo resultado de órgãos, pois secretarias são órgãos)
      const manifestationsBySecretaria = bySecretaria
        .map(r => ({ key: r.orgaos ?? 'Não informado', count: r._count._all }))
        .sort((a, b) => b.count - a.count);
      ...
      return {
        ...
        manifestationsByDay,
        manifestationsByStatus: statusCounts,
        ...
        filters: { servidor, unidadeCadastro }
      };
    } catch (error) {
